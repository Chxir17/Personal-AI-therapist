...
public enum Prompts {
    ...
    FIND_ARTICLE_PROMPT(
            ...
            - Если он медицинский и в нём явно или неявно содержится тема из одной из статей — верни точное название статьи (оно должно точно совпадать с заголовком).
            - Если ни одна статья не подходит — верни строго: no
            [Список статей]:
            """ + HypertensionQA.getAllQuestions() + """
            
            [Формат ответа]:
            - Только строка: название статьи (без JSON, без кавычек)
            - Или: no
            """
    );
