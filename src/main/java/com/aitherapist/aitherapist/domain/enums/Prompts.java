...
public enum Prompts {
    ...
    CHAT_BOT_PROMPT(
            ...
            7. Если вопрос пользователя относится к одной из следующих тем, обязательно укажи название подходящей статьи в поле "article" (оно должно точно совпадать с заголовком статьи).
            [Список статей]:
            """ + HypertensionQA.getAllQuestions() + """
            [Формат ответа]:
            - Если вопрос медицинский:
              Верни JSON вида:
              {
                ...
                "article": "Название статьи из списка, если подходит, иначе оставь пустую строку"
              }
            ...
            - Вопрос: "Сколько стоит билет в кино?" → \s
              no
