...
public enum Prompts {
    ...
    RECOMMENDATION_SCORE_BETA_PROMPT(
        ...
        [Задача]: Проанализируй медицинские данные пациента и цели, которые установил врач, и напиши ему личную рекомендацию основанную на целях по показателям для улучшению здоровья. В рекомендациях используй формальный стиль обращения, акцентируй внимание на сильных и слабых сторонах здоровья пациента, давай советы по основным аспектам жизни (питание, активность, режим дня). Дай совет как приблизиться к рекомендациям врача. Сообщение должно быть структурировано и не превышать 600 символов без пробелов. На основании составленной рекомендации выбери из списка названий статей наиболее подходящую. Оформи рекомендацию красиво, добавь смайлики.
        
        Дополнительно:
        - Учитывай личные цели пациента, которые будут ниже где 1 часть — название показателя (например, «Вес», «Давление», «Глюкоза»), а значение — целевое значение или состояние (например, «снизить до 70 кг», «норма 120/80», «глюкоза ≤5,5»).
        - Обязательно анализируй расхождения между текущими показателями и целями пациента и включай рекомендации по их достижению.
        - Если целей нет, пиши стандартную рекомендацию только на основе медицинских данных.
        ...
        3. Сделай краткий обзор общего состояния здоровья.
        4. Дай персонализированные рекомендации по таким категориям, как питание, физическая активность, распорядок дня и возрастные особенности.
        5. Отдельно укажи рекомендации, связанные с достижением целей пациента (если они есть).
        6. Если значения отклоняются от нормы или целей, подробно объясни причины и предложи корректирующие действия.
        7. Обязательно придерживайся указанного лимита по длине текста рекомендации.
        8. Выбери наиболее подходящую статью строго из предоставленных.
        ...
        [Список статей]:
        """ + HypertensionQA.getAllQuestions() + """
        [Примечания]: Убедись, что рекомендации четко адресованы конкретному человеку, учитывают его возраст, состояние здоровья и индивидуальные цели.
        [Критерии качества]:
        - Четкость и однозначность рекомендаций.
        - Адекватный учет индивидуальных особенностей пациента и его целей.
        - Корректность медицинских комментариев и предложений.
        - Соблюдение ограничений по длине текста.
        """
    ),
